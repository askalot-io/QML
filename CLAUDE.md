...
**The `askalot_qml/` package is a synced mirror — never modify it in this repo.** New QML-adjacent code lives in sibling top-level packages (e.g. `askalot_inventory/`, which owns the PDF-to-inventory pipeline and the `askalot-inventory` CLI).
...
- **`build-inventory`** — PDF → structured question inventory. Step 1 runs the `askalot-inventory` CLI (in the sibling `askalot_inventory` package) which rasterizes each PDF page with `pdftoppm` and extracts verbatim text via Claude Sonnet 4.6 vision on AWS Bedrock, producing a `<stem>_text.md` intermediate. Steps 2-3 are the per-section subagent extraction + judgement-agent verification, reading from that markdown.
- **`inventory-to-qml`** — Question inventory → a single QML file (multiple blocks) with judgement agent verification
...
`evaluation/` contains converted questionnaires in per-questionnaire subfolders:

```
evaluation/<category>/SURVEY_NAME/
  source.pdf                         # Original (keep original filename)
  SURVEY_NAME_text.md                # Claude-vision text extraction (Phase 1, via askalot-inventory)
  SURVEY_NAME_question_inventory.md  # Produced by `build-inventory` (Phase 2)
  SURVEY_NAME.md                     # Analysis report (Phase 4)
  SURVEY_NAME.qml                    # Single QML output (Phase 3)
```

The current `askalot-inventory` tool rasterizes each PDF page with `pdftoppm` and extracts verbatim text via **Claude Sonnet 4.6 vision on AWS Bedrock**, writing `SURVEY_NAME_text.md` next to the source and caching under `~/.cache/askalot_qml/inventory/<hash>/`. The `build-inventory` skill reads that intermediate.

**Prerequisites for the extractor:**
- `pdftoppm` on PATH (`apt install poppler-utils`).
- AWS credentials in the ambient environment (e.g. via `aws sso login`) with `bedrock:InvokeModel` access to `us.anthropic.claude-sonnet-4-6` in the target region. Region resolves from `AWS_REGION` / `AWS_DEFAULT_REGION` unless overridden with `--aws-region`.

**Historical extracts:** Some older conversions still contain `SURVEY_NAME_ocr.md` (from a `pdftotext` + Tesseract pipeline) or `SURVEY_NAME_docling.md` (from a Docling pipeline). NPHS keeps both as benchmark references for extractor comparison. Treat these as read-only historical artifacts — the current pipeline writes `_text.md`.
...
**QML output is a single file per questionnaire.** The survey's original sections become *blocks* inside that one QML file, ordered so producer blocks come before consumer blocks. A questionnaire is never split across multiple QML files. Variables and item outcomes flow naturally across blocks because they share one `codeInit` scope and one dependency graph, so Z3 can reason about reachability end-to-end without cross-file hand-overs.

**Historical corpus note:** Some older conversions under `evaluation/` still contain multiple `NN_section_name.qml` files per questionnaire from an earlier pipeline version. Treat these as read-only historical artifacts — do not merge them retroactively. All *new* conversions produce a single `SURVEY_NAME.qml`.
